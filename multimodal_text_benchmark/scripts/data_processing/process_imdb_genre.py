...
parser = argparse.ArgumentParser(
        description='Process the IMDB Dataset from Kaggle.')
...
train_data.to_csv(os.path.join(args.dir_path, args.output_subdir, train_name), index=False)
test_data.to_csv(os.path.join(args.dir_path, args.output_subdir, test_name), index=False)
...
predictor = TabularPredictor(label=label, path=os.path.join(args.dir_path, args.output_subdir), problem_type=problem_type)
